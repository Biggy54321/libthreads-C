...
void *thread_main(void *main_arg) {
    ...
    /* Test 1 */
    print_str("Test 1: Thread returning from the start function\n");
    /* Create the thread */
    debug_str("thread_main() created ret_thread()\n");
    thread_create(&td, ret_thread, NULL);
    ...
    /* Test 2 */
    print_str("Test 2: Thread exiting from the some subfunction\n");
    /* Create the thread */
    debug_str("thread_main() created exit_thread()\n");
    thread_create(&td, exit_thread, NULL);
    /* Join with the thread to get the return status */
